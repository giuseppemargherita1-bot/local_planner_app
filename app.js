...
function sumAllocationWeights(allocations) {
  return (allocations || []).reduce((acc, allocation) => acc + allocationWeight(allocation), 0);
}
...
const RESOURCE_COLUMN_LABELS = [
  ...
  "Datore lavoro", "Livello ass.", "Tipo", "€/h straord", "€/G Pres. Off", "€/G Pres. Cant", "PB +FISSO", "€/h Off.", "€/h Cant.", "GLOB +FISSO",
...
function isOverallProjectCode(projectCode = "") {
  ...
  return overalls.some((o) => String(o?.name || "").trim().toUpperCase() === code || String(o?.id || "").trim().toUpperCase() === code);
}

function buildOverallOfficina() {
  const officina = (state.projects || []).filter((p) =>
    String(p.code || "").toUpperCase().includes("OFFICINA")
  );
  ...
  state.overalls.OVERALL_OFFICINA = {
    ...
    project_codes: officina.map((p) => p.code),
  };
}

function createCustomOverall(name, projectCodes) {
  state.overalls = state.overalls || {};
  const used = Object.values(state.overalls).flatMap((o) => o.project_codes || []);
  const alreadyUsed = (projectCodes || []).filter((p) => used.includes(p));
  if (alreadyUsed.length > 0) {
    throw new Error("Alcune commesse sono già assegnate ad un altro overall");
  }
...
function normalizeResourceAllocations(resourceId) {
  const allocs = state.allocations.filter((a) => Number(a.resource_id) === Number(resourceId));
  const byWeek = {};
  allocs.forEach((a) => {
    for (let w = Number(a.week_from); w <= Number(a.week_to); w += 1) {
  ...
  Object.keys(byWeek).forEach((week) => {
    ...
    if (list.length > 2) {
      throw new Error("Una risorsa non può avere più di 2 allocazioni nella stessa settimana");
    }
    if (list.length === 2) {
      list.forEach((a) => {
        a.weight = 0.5;
...
function plannerProjectMeta(projectId, fallbackProjectName = "") {
  ...
  if (project?.type === "WS") {
    const overall = state.projects.find((p) => p.code === "OVERALL OFFICINA");
    return {
      project_id: overall ? Number(overall.id) : Number(projectId),
      project: "OVERALL OFFICINA",
    };
  }
  return {
    project_id: Number(projectId),
    project: fallbackProjectName || project?.code || "",
  };
}
...
function mansioneHoverText(resource, allocation, weekFrom, weekTo) {
  const role1 = (resource?.role1 || "").trim();
  const role2 = (resource?.role2 || "").trim();
  const resRole = [role1, role2].filter(Boolean).join(" / ");
  const allocRole = (allocation?.role || "").trim();
  const projectId = allocation?.project_id ? Number(allocation.project_id) : null;
  const range = weekFrom && weekTo ? `W${weekFrom}-W${weekTo}` : weekFrom ? `W${weekFrom}` : "";
  const noDemand = projectId && allocRole
    ? !demandHasRoleInRange(projectId, allocRole, weekFrom, weekTo)
    : false;
  const noDemandSuffix = noDemand ? " - NO FABBIS" : "";
  if (!resRole) return range ? `${range}` : "";
  if (allocRole && roleMatches(resource || {}, allocRole)) {
    return `${range} - ${resRole} - MANSIONE OK${noDemandSuffix}`;
  }
  if (allocRole) {
    return `${range} - ${resRole} - FUORI MANSIONE - ${allocRole}${noDemandSuffix}`;
  }
  return `${range} - ${resRole}${noDemandSuffix}`;
}
...
function plannerCellBadge(projectId, role, week, required, allocated) {
  ...
  const noFab =
    ...
    !demandHasRoleInWeek(projectId, role, week);
...
function plannerCellWarningText(projectId, role, week) {
  const allocations = state.allocations.filter(
    (a) =>
      Number(a.project_id) === Number(projectId) &&
      normalizeRoleKey(a.role) === normalizeRoleKey(role) &&
  ...
  const requiredRow = state.demands.find(
    (d) =>
      Number(d.project_id) === Number(projectId) &&
      normalizeRoleKey(d.role) === normalizeRoleKey(role) &&
      Number(d.week) === Number(week)
  );
  const required = Number(requiredRow?.qty || 0);
  const allocated = sumAllocationWeights(allocations);
  const noFab = !demandHasRoleInWeek(projectId, role, week) || allocated > required;
  const rows = allocations.map((allocation) => {
    const resource = state.resources.find((r) => Number(r.id) === Number(allocation.resource_id));
    const segFuori = warningLabelsForWeek(allocation, week).some((w) =>
      String(w || "").toUpperCase().includes("FUORI MANSIONE")
    );
    const parts = [allocation.resource || resource?.name || "RISORSA"];
    if (segFuori) parts.push("FUORI MANSIONE");
    if (noFab) parts.push("NO FABB");
    return parts.join(" - ");
  });
  return rows.join(", ");
}
...
function renderGanttConflictBox() {
  ...
  if (!items.length) {
    ...
    ganttConflictBox.innerHTML = `
      <div class="selection-sub" style="margin-bottom:8px;font-weight:700;">⚠️ Risorsa gia allocata nella settimana selezionata</div>
...
async function saveGanttActionModal() {
  ...
  if (ganttProjectSelect.value === "INDISP") {
    ...
    return;
      ...
      const project = state.projects.find((p) => p.code === ganttProjectSelect.value);
      if (!project) throw new Error("Commessa non trovata.");
      if (!isOverallProjectCode(project.code)) {
        alert("Le allocazioni devono essere fatte su OVERALL");
        return;
      }
      ...
      if (ganttActionState.mode === "allocation" && ganttActionState.allocation_id) {
        ...
        await loadAll(true);
        try {
          normalizeResourceAllocations(resource.id);
          await persistResourceAllocationWeights(resource.id);
        } catch (e) {
          alert(e.message);
          return;
        }
...
function fillProjectFilter() {
  const projects = [...new Set(state.projects.map((p) => p.code).filter(Boolean))].sort((a, b) => {
...
function projectSortWeekByCode(projectCode) {
  ...
  return project ? projectSortWeek(project.id) : 999;
}
...
function renderSummary() {
  ...
  demandSourcePath.textContent = state.current_demand_path || state.default_demand_path || "";
}
...
function computePlannerRows(projectFilter = "", roleFilter = "", includeZeroDemandProjects = false, updateHiddenNoticeFlag = false) {
  ...
  state.demands.forEach((d) => {
    ...
    const key = `${meta.project_id}|${d.role}`;
    if (!combos.has(key)) {
      combos.set(key, { project_id: meta.project_id, project: meta.project, role: d.role });
    }
    const qty = Number(d.qty || 0);
    demandMap.set(`${key}|${d.week}`, qty);
    if (Number(d.week) >= firstVisibleWeek && qty > 0) {
      activeProjects.add(meta.project_id);
    }
  });
  ...
  state.allocations.forEach((a) => {
    ...
    const key = `${meta.project_id}|${a.role}`;
    const isOverallOfficina = project?.code === "OVERALL OFFICINA";
    if (!combos.has(key)) {
      combos.set(key, { project_id: meta.project_id, project: meta.project, role: a.role });
    }
    for (let week = Number(a.week_from); week <= Number(a.week_to); week += 1) {
      ...
      if (week >= firstVisibleWeek) activeProjects.add(meta.project_id);
    }
  });
  ...
  let rows = [...combos.values()].map((combo) => {
    const weeks = Array.from({ length: 52 }, (_, idx) => {
      const week = idx + 1;
      const req = Number(demandMap.get(`${combo.project_id}|${combo.role}|${week}`) || 0);
      const allocsRaw = allocationsByKey.get(`${combo.project_id}|${combo.role}|${week}`) || [];
      ...
      const allocated = sumAllocationWeights(allocs);
      return {
        week,
        required: req,
        allocated,
  ...
  const hiddenProjects = [...new Set(rows.filter((row) => !activeProjects.has(row.project_id)).map((row) => row.project_id))];
  if (!includeZeroDemandProjects) {
    rows = rows.filter((row) => activeProjects.has(row.project_id));
  }
  if (updateHiddenNoticeFlag) updatePlannerHiddenNotice(hiddenProjects.length);
  rows.sort((a, b) => {
    const pa = projectSortWeek(a.project_id);
    const pb = projectSortWeek(b.project_id);
...
function computeDemandOnlyRows(projectFilter = "", roleFilter = "", includeZeroDemandProjects = true) {
  ...
  state.demands.forEach((d) => {
    ...
    const meta = plannerProjectMeta(d.project_id, d.project);
    const key = `${meta.project_id}|${d.role}`;
    if (!combos.has(key)) combos.set(key, { project_id: meta.project_id, project: meta.project, role: d.role });
    const qty = Number(d.qty || 0);
    demandMap.set(`${key}|${d.week}`, qty);
    if (Number(d.week) >= firstVisibleWeek && qty > 0) activeProjects.add(meta.project_id);
  });

  let rows = [...combos.values()].map((combo) => {
    const weeks = Array.from({ length: 52 }, (_, idx) => {
      const week = idx + 1;
      return { week, required: Number(demandMap.get(`${combo.project_id}|${combo.role}|${week}`) || 0) };
    });
    return {
      ...combo,
      weeks,
  ...
  const rf = (roleFilter || "").trim();
  rows = rows.filter((row) => (!pf || row.project === pf) && (!rf || row.role === rf));
  if (!includeZeroDemandProjects) rows = rows.filter((row) => activeProjects.has(row.project_id));
  rows = rows.filter((row) => row.totalRequired > 0 || includeZeroDemandProjects);
  rows.sort((a, b) => projectSortWeek(a.project_id) - projectSortWeek(b.project_id) || a.project.localeCompare(b.project) || a.role.localeCompare(b.role));
  return rows;
}
...
function plannerWeeklyTotals(rows) {
  const totals = new Map();
  rows.forEach((row) => {
    const isOverallLocked = String(row.project || "").toUpperCase() === "OVERALL OFFICINA";
    row.weeks
      .filter((w) => w.week >= getCurrentWeek())
      .forEach((w) => {
        if (!totals.has(w.week)) {
          totals.set(w.week, { week: w.week, required: 0, allocated: 0, residual: 0 });
        }
  ...
  return visibleWeeks().map((week) => {
    const item = totals.get(week) || { week, required: 0, allocated: 0, residual: 0 };
    return {
      ...item,
                ...
                .map((week) => {
                  ...
                  const cls =
                    residual > 0
                      ...
                      ? "cell-covered"
                      : "cell-empty";
                  const badge = plannerCellBadge(row.project_id, row.role, week, req, alloc);
                  ...
                  const warningText = plannerCellWarningText(row.project_id, row.role, week);
                  ...
                  return `
                    <td>
                      ...
                      >
                        ...
                        <span class="metric">A ${alloc}</span>
...
function selectTarget(target, resourceId = null) {
  ...
  demandQty.value = String(currentDemandQty(target.project_id, target.role, baseWeek));
      ...
      ${row.weeks
          ...
          const cls =
              w.residual > 0
                ...
                ? "cell-covered"
                : "cell-empty";
          const badge = plannerCellBadge(row.project_id, row.role, w.week, w.required, w.allocated);
          const noFabClass = badge?.cls === "badge-no-fab" ? "cell-warning-nofab" : "";
          ...
          const diffClass = diffOk ? "cell-diff" : "";
          const diffTitle = diffOk ? ` Delta fabb ${diff.prev_qty}->${diff.new_qty}` : "";
          const warningText = plannerCellWarningText(row.project_id, row.role, w.week);
          ...
          return `
            <td>
              <button
                ...
                data-role="${escapeHtml(row.role)}"
                data-week="${w.week}"
                ${isOverallLocked ? 'disabled title="OVERALL OFFICINA non modificabile manualmente"' : ""}
                ${names}
              >
                ${cellBadgeHtml(badge)}
                <span class="metric">R ${w.required}</span>
...
function resourceCardClass(resource, selectedWeekFrom = null, selectedWeekTo = null, role = "") {
  ...
  const conflicts = weekFrom !== null && weekTo !== null ? allocationConflict(resource.id, weekFrom, weekTo) : [];
  const unavailable = weekFrom !== null && weekTo !== null ? unavailabilityConflict(resource.id, weekFrom, weekTo) : [];
  if (!resourceHasAnyRole(resource)) return "off-role";
  if (!resourceActive(resource, weekFrom)) return "inactive";
  if (unavailable.length > 0) return "busy";
...
function renderAssignResourceOptions(preselectedId = null) {
  ...
  const options = scopeResources
    .map((r) => {
      ...
      const conflict = allocationConflict(r.id, weekFrom, weekTo);
      const unavailable = unavailabilityConflict(r.id, weekFrom, weekTo);
      const hasRole = resourceHasAnyRole(r);
      const roleOk = !role || roleMatches(r, role);
      const active = resourceActive(r, weekFrom);
      const labels = [];
      ...
      if (conflict.length > 0) labels.push("gia allocato");
      const suffix = labels.length ? ` | ${labels.join(", ")}` : "";
      const sel = selectedSet.has(Number(r.id)) ? "selected" : "";
      const extTag = useExt ? " | EXT" : "";
      let optionColor = "#111a2a";
      let optionWeight = "700";
      if (!active) {
        optionColor = "#8a94a8";
        optionWeight = "500";
      } else if (unavailable.length > 0 || conflict.length > 0) {
        optionColor = "#b42318";
        optionWeight = "700";
      }
...
function renderSelectionAllocations() {
  ...
  const rows = state.allocations
    .filter(
      (a) =>
        Number(a.project_id) === Number(selectedTarget.project_id) &&
...
function reportScopeProjects() {
  if (reportBuilderState.aggregateScope === "OVERALL_CANTIERI") {
    return new Set(reportEligibleProjects().filter((p) => p.code !== "OVERALL OFFICINA").map((p) => p.code));
  }
  if (reportBuilderState.aggregateScope === "OVERALL_GENERALE") {
    return new Set(reportEligibleProjects().map((p) => p.code));
  }
  if (reportBuilderState.projectFilter) return new Set([reportBuilderState.projectFilter]);
  return new Set(reportEligibleProjects().map((p) => p.code));
}
...
function countDistinctAllocationsForWeek(week, projectCodes = null) {
  ...
  state.allocations.forEach((allocation) => {
    ...
    if (!inWeek) return;
    if (projectCodes && !projectCodes.has(allocation.project)) return;
...
function countAllocationsForWeek(week, projectCodes = null) {
  return sumAllocationWeights(state.allocations.filter((allocation) => {
    const inWeek = Number(allocation.week_from) <= week && Number(allocation.week_to) >= week;
    if (!inWeek) return false;
    return !projectCodes || projectCodes.has(allocation.project);
  }));
}
  ...
  if (mode === "mansione") {
    ...
    rows.forEach((row) => {
      ...
      const allocations = state.allocations.filter((allocation) =>
        allocation.project === row.project &&
...
function computeUtilizationSeries() {
  ...
  return weekGroups.map((group) => {
    const activeValues = group.weeks.map((week) => activeMap.get(week) || 0);
    const allocatedValues = group.weeks.map((week) => countAllocationsForWeek(week, scopeProjects));
    const active = Math.round(activeValues.reduce((a, b) => a + b, 0) / Math.max(activeValues.length, 1));
    const allocated = Math.round(allocatedValues.reduce((a, b) => a + b, 0) / Math.max(allocatedValues.length, 1));
    return {
      label: group.label,
      active,
      allocated,
      unallocated: Math.max(active - allocated, 0),
    };
  });
}
...
function buildUtilizationChart(series, options = {}) {
  ...
  return {
    ...
    legendItems,
    summaryCards: [
      { label: "In forza medio", value: String(Math.round(totals.active / Math.max(series.length, 1))) },
      { label: "Allocati medi", value: String(Math.round(totals.allocated / Math.max(series.length, 1))) },
      { label: "Non allocati medi", value: String(Math.round(totals.unallocated / Math.max(series.length, 1))) },
    ],
  };
}
...
function setDemandPathOverride(path) {
  try {
    if (path) window.localStorage.setItem("demand_path_override", path);
  } catch {
  ...
  if (ganttGroupMode === "project") {
    ...
    state.allocations
      ...
      .forEach((a) => {
        ...
        allocCounts.set(key, (allocCounts.get(key) || 0) + allocationWeight(a));
      });
...
function renderTimelineFallback() {
  ...
  const resourceSortArrow = isResourceSort ? (ganttFallbackSort.dir === "desc" ? "▼" : "▲") : "⇅";
  const weeks = visibleWeeks();
  const weekHeads = weeks
    .map((w) => {
      ...
      const arrow = isWeekSort ? (ganttFallbackSort.dir === "asc" ? "▲" : "▼") : "⇅";
      return `
...
async function saveAssignment({ project_id, project, role, resource_id, week_from, week_to }, { refresh = true } = {}) {
  ...
  if (!resource) throw new Error("Risorsa non trovata.");
  if (!isOverallProjectCode(project)) throw new Error("Le allocazioni devono essere fatte su OVERALL");
  ...
  const merge = external
    ...
    : state.allocations.find(
        (a) =>
          Number(a.project_id) === Number(project_id) &&
  ...
  if (!external) {
    const overlaps = allocationConflict(resource_id, week_from, week_to).filter((a) => {
      if (merge && Number(a.id) === Number(merge.id)) return false;
      return true;
    });
  ...
  if (merge) {
    ...
    await api("/api/allocations", {
      method: "POST",
      body: JSON.stringify({ project_id, project, role, resource_id, week_from, week_to, weight: 1 }),
    });
  }
  logUserAction(
    "Allocazione",
    `${resource.name} -> ${project} | ${role} | W${week_from}-W${week_to}`
  );
  if (refresh) {
    await loadAll();
  }
...
async function handleAssignSave() {
  ...
  for (const resource_id of resourceIds) {
    ...
    try {
      normalizeResourceAllocations(resource_id);
      await persistResourceAllocationWeights(resource_id);
    } catch (e) {
      alert(e.message);
...
async function handleDemandSave() {
  ...
  if (String(selectedTarget.project || "").toUpperCase() === "OVERALL OFFICINA") {
    setStatus("OVERALL OFFICINA non è modificabile manualmente.");
    return;
  }
...
async function loadAll(silent = false) {
  ...
  state = {
    ...
    current_demand_path: data.current_demand_path || data.default_demand_path || "",
  ...
  try {
    const resourceIds = [...new Set((state.allocations || []).map((a) => Number(a.resource_id)).filter((x) => x > 0))];
    resourceIds.forEach((resourceId) => normalizeResourceAllocations(resourceId));
  } catch (err) {
    setStatus(err.message || "Errore normalizzazione allocazioni");
  }
  fillRoleList();
  fillProjectFilter();
...
safeOn(importDemandsBtn, "click", async () => {
  try {
    ...
    const storedPath = getDemandPathOverride();
    const basePath = storedPath || state.current_demand_path || state.default_demand_path;
...
safeOn(openDemandSourceBtn, "click", async () => {
  try {
    await api("/api/open-demand-source", {
      method: "POST",
      body: JSON.stringify({ demand_path: state.current_demand_path || state.default_demand_path }),
    });
...
safeOn(importPlanningBtn, "click", async () => {
  try {
    ...
    await api("/api/import-resource-planning", {
      method: "POST",
      body: JSON.stringify({
        demand_path: state.current_demand_path || state.default_demand_path,
